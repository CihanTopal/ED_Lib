...
cv::Mat EDCircles::drawResult(bool onImage, ImageStyle style)
{
	...
	if (style == ImageStyle::ELLIPSES|| style == ImageStyle::BOTH)
		...
		{
            double degree = (ellipses[i].theta * 180) / CV_PI; // convert radian to degree (opencv's ellipse function works with degree)
...
{
    double prec = CV_PI / 16;  // Alignment precision
